...
**Note:** If the HTTP request is canceled before you get the response, the Sinch platform will try to stop the call from initating. However, in some rare cases, it is still possible to receive a call. This depends on how far along the initialization phase the request is canceled.
